...
+ (void) setSdkPlatform:(NSString *)platform {
    NSUserDefaults *preferences = [NSUserDefaults standardUserDefaults];
    [preferences setObject:platform forKey:ND_SDK_PLATFORM_CUSTOM];
    
    [NDLogger debug:@"Storing value %@ stored: %@", ND_SDK_PLATFORM_CUSTOM, platform];
...
+ (NSString *) getSdkPlatform {
    ...
    if ([preferences objectForKey:ND_SDK_PLATFORM_CUSTOM] == nil) {
        response = ND_SDK_PLATFORM;
    } else {
        response = [preferences stringForKey:ND_SDK_PLATFORM_CUSTOM];
        [NDLogger debug:@"Value found in storage: %@", response];
    }
